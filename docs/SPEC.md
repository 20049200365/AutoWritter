...
2. **写作阶段**：以章为单位的流水线 —— 上下文检索 → 细纲生成+人工确认 → 扩写生成 → 评审 Agent 打分 → 人工接受/驳回 → 驳回迭代（两阶段生成，已决策）。
...
```
            ┌─────────────────── 章 N 生成任务（两阶段，已决策）───────────────────┐
用户触发 ──► │ ① 上下文组装 ──► ② 细纲生成 ──► ③ 人工确认细纲 ──► ④ 扩写生成 ──► ⑤ 评审打分 ──► ⑥ 人工决策 │
            │  (检索服务)     (节拍/实体/    (所见即所得可改，  (注入Skill，   (分维度+意见)  接受│驳回 │
            │                 伏笔清单)       可跳过)          按细纲扩写)                 │
            └──────────────────────────────────────────────────────────┬───┘
                                          接受 ──► 后处理流水线（§5.4）│
                                          驳回 ──► 记录偏好事件 ──► 情节方向错→回退③；文笔问题→重扩写
```

① **上下文组装**（详见 §5.3）：前 K 章全文（必装）、前文摘要与前章结尾、本章大纲路径、相关人物档案+实体图子图、活跃伏笔、世界观词条、用户偏好档案，按 token 预算分级装配。
② **细纲生成**：写手基于装配产出本章细纲 = 情节节拍（3~6 条）+ 涉及实体清单 + 伏笔动作；落库供确认。
③ **人工确认细纲**：细纲所见即得可编辑/确认/跳过；**方向性错误在这一步纠正，避免浪费整篇正文**。
④ **扩写生成（写手 Agent）**：**Skill 注入仅在此阶段**；输入含已确认细纲，流式输出草稿（不直接入正典）。
⑤ **评审 Agent**：独立模型/独立提示词，按维度打分（情节连贯、人物一致性、伏笔照应、节奏、文风贴合）并输出结构化意见；评审权重受用户画像动态调整。
⑥ **人工决策**：接受 → 草稿定稿并触发后处理；驳回 → 驳回理由（标签+自由文本）分路处理：情节方向错回退细纲确认、文笔问题直接重扩（全部版本留档）。
...
### 3.4 日志规范（横切要求，全模块必须遵守）
- **基建**：Python 标准库 logging，统一初始化入口；同时输出到控制台与 `~/.novelstudio/logs/novelstudio.log`（按大小滚动，默认保留 5 份）
- **关联 ID**：日志记录必须携带模块名与相关上下文 ID（project_id / chapter_id / task_id / session_id，存在即带），便于跨模块追踪一条流水线
- **级别约定**：DEBUG=过程细节（装配每层命中、检索两路结果、工具调用入参）；INFO=关键状态变迁（任务阶段切换、接受/驳回、建议采纳、事件派发）；WARNING=降级/压缩/兑底触发；ERROR=失败（带 resumable 标记与堆栈）
- **模块覆盖**：M1 写操作与事件派发、M2 检索命中与融合、M3 流水线阶段/SSE 事件/LLM 调用（模型/耗时/token 用量）、M6 请求行、M8 队列步骤，均需达到上述级别要求
- **敏感信息红线**：API key 永不入日志；正文/草稿不记全文，只记长度与首尾少量字符；用户驳回反馈可记（是产品数据）
- **验收**：每个模块的验收表含一条日志断言（关键操作产生带关联 ID 的日志，且日志文件无敏感信息）
...
- 章接受 → M8 自动生成/更新章摘要（≤200 字），存 chapters.summary。
- **不设全书/卷级 AI 摘要**（已决策）：大模型上下文（128k~1M）充裕，K 章之外直接装章摘要即可，无需再分层压缩。
- 写第 N 章时，前文记忆 = 前 K 章全文（必装）+ K 章之外的章摘要 + 第 N-1 章结尾原文（~1500 字，保证衔接语感）+ 实体驱动检索命中的更早原文块。

### 5.3 单章上下文组装（写手输入的组织方式）

按 **优先级 × token 预算**自顶向下装配（默认预算 128k，可配置至模型上限）。现代大模型上下文充裕，预算是**成本与注意力质量的软约束，不是空间硬约束**；分层的意义在于按相关性排序，压缩仅在逼近上限时兜底。每层带账本记录（写入 `generation_tasks.context_snapshot`，前端可预览"本章 AI 看到了什么"）：

| 优先级 | 内容 | 来源 | 预算占比 |
|---|---|---|---|
| P0 | 用户偏好档案 + 激活 Skill 提示词 | preference_profile / skills | ~5% |
| P1 | 本章大纲路径（卷概要→篇章→L3 节拍）+ 前章结尾原文（衔接锚点）+ K 章之外的更早章摘要 | outline_nodes / chapters | ~20% |
| P2 | **前 K 章全文**（prior_full_k 默认 3，必装不裁剪）+ 活跃伏笔清单 + 涉及人物档案与实体图子图（沿隶属/持有等边顺带捞相邻世界观词条） | chapters / foreshadows / characters / relations / world_entries | ~40% |
...
超预算时从 P4 向 P2 依次压缩（原文块减量 → 词条减量 → 人物卡只留核心字段），P0/P1 不压缩；P2 前 K 章全文为必装项，装不下时报错提示调小 K（不静默裁剪）。两阶段生成（M3 §3.1）：细纲阶段用 P0~P2，扩充阶段才加 P3/P4 并注入 Skill。
...
章节定稿 ──► 1. 字数/状态落库
          ...
          ─► 6. 伏笔提议：埋设发现（草稿可能新埋了伏笔）与回收提议（某伏笔时机适合回收）→ 建议消息；
              用户采纳后才执行对应管理操作，AI 不直接变更伏笔状态（Agent 能力边界止于提议）
...
**主要风险**：
1. **长文一致性**是产品成败核心 —— 缓解：章摘要 + 上下文账本可视化 + 评审 Agent 专职核对 + 一致性巡检。
2. **长文注意力稀释与成本**（大上下文不等于好效果，无关内容干扰生成且按 token 计费）—— 缓解：P0~P4 相关性排序，预算软上限可配，P4 层只进实体共现命中块。
